...
class TrainAdmin(admin.ModelAdmin):
    list_display = ('id', 'user', 'course_section', 'training_type', 'can_delete')
    list_filter = ('training_type', 'can_delete')
    search_fields = ('descriptions', 'user__username', 'course_section__name')
    list_editable = ('can_delete', 'training_type')
    ordering = ('-id',)
    readonly_fields = ('media_file',)
    fieldsets = (
        ("General Information", {
            'fields': ('descriptions', 'course_section', 'user', 'training_type', 'can_delete')
        }),
        ("Media", {
            'fields': ('media_file',)
        }),

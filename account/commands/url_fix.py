    ...
    def handle(self, *args, **options):
        ...
        for app in apps:
            if "urls" not in get_files(app):
                ...
                time.sleep(1)
                os.popen(
                    f"""echo "from {app}.urls import *" > {PROJECT_DIR}/{app}/urls/v1/__init__.py"""
                )
                os.popen(
                    f"""echo "from .urls import *" > {PROJECT_DIR}/{app}/urls/__init__.py"""
                )
                os.popen(
                    f"""echo "from django.urls import path \nurlpatterns = [\n]" >  {PROJECT_DIR}/{app}/urls/v1/urls.py"""
                )
                os.popen(
                    f"""echo "from django.urls import path, include\napp_name = '{app}'\nurlpatterns = [\n\tpath('api/v1/', include('account.urls.v1.urls')),\n]\n" >  {PROJECT_DIR}/{app}/urls/urls.py"""
                )
                os.popen(f"touch {PROJECT_DIR}/{app}/urls/v1/views.py")
                os.popen(
                    f"""echo "from rest_framework import serializers\n#create your serializers" > {PROJECT_DIR}/{app}/urls/v1/serializers.py"""
                )
        # for app in apps:
